...
public interface CheckLiveTaskDetailRepository extends JpaRepository<CheckLiveTaskDetail,String> {
    ...
    @Modifying
    @Query(value = "UPDATE check_live_task_detail SET sfzg=?2,  ryyz=?3,  dzwl=?4 where id= ?1",nativeQuery = true)
    void checkDgdwUpdate(Long id, String sfzg, String ryyz, String dzwl);
    @Modifying
    @Query(value = "UPDATE check_live_task_detailxs SET sfzg=?2,  ryyz=?3,  dzwl=?4 where id= ?1",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE check_live_task_detail SET dydj=?2,  yhxx=?3,  czfa=?4 ,qtwt=?5 where id= ?1",nativeQuery = true)
    void checkQuestionUpdate(String detailId, String dydj, String yhxx, String czfa, String qtwt);
    @Modifying
    @Query(value = "UPDATE check_live_task_detailxs SET  dydj=?2,  yhxx=?3,  czfa=?4 ,qtwt=?5 where id= ?1",nativeQuery = true)
