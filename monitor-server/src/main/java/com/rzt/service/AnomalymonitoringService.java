...
public class AnomalymonitoringService extends CurdService<Anomalymonitoring, AnomalymonitoringRepository> {
    ...
    public WebApiResponse xsTu() {
        ...
        String sql0 = "SELECT " +
                ...
                "  x.TD_ORG " +
                "FROM xs_txbd_task x LEFT JOIN ANOMALY_MONITORING a ON a.TASKID = x.ID " +
                "WHERE (stauts = 0 OR stauts = 1) AND trunc(PLAN_START_TIME) < trunc(sysdate) " +
                "      OR a.ONECHECK_STATUS = 0 " +
                "GROUP BY x.TD_ORG";
        /**
         * 二级正常巡视任务超期警告
         */
        String sql1 = "SELECT " +
                ...
                "WHERE (stauts = 0 OR stauts = 1) AND trunc(plan_end_time) < trunc(sysdate) " +
                "      AND (ceil((sysdate - PLAN_START_TIME) * 24 * 60) < 40 OR a.ONECHECK_STATUS = 0) " +
                "GROUP BY x.TD_ORG";
        /**
         * 正常巡视任务未上线 二级
         */
        String sql2 = "SELECT " +
                ...
                "FROM XS_ZC_TASK x " +
                " LEFT JOIN ANOMALY_MONITORING a ON a.TASKID = x.ID " +
                "WHERE (ceil((sysdate - PLAN_START_TIME) * 24 * 60) < 40 OR a.ONECHECK_STATUS = 0) " +
                "GROUP BY x.TD_ORG";
        /**
         * --保电巡视任务未上线 二级
         */
        ...
        String sql5 = "SELECT " +
                ...
                " LEFT JOIN ANOMALY_MONITORING a ON a.TASKID = x.ID " +
                "WHERE trunc(plan_start_time) = trunc(sysdate) AND plan_start_time < nvl(real_start_time, sysdate) " +
                "      AND (ceil((sysdate - PLAN_START_TIME) * 24 * 60) < 40 OR a.ONECHECK_STATUS = 0) GROUP BY x.TD_ORG";
        ...
        String sql7 = "SELECT sum(count) sum,xxx.ID as TD_ORG FROM( " +
                ...
                "JOIN RZTSYSDEPARTMENT d ON d.ID = xx.TD_ORG  GROUP BY d.ID) xxx GROUP BY xxx.ID";
        /*所有任务*/
        List<Map<String, Object>> list0 = this.execSql(sql0);
        List<Map<String, Object>> list1 = this.execSql(sql1);
        List<Map<String, Object>> list2 = this.execSql(sql2);
        List<Map<String, Object>> list3 = this.execSql(sql3);
        List<Map<String, Object>> list4 = this.execSql(sql4);
        List<Map<String, Object>> list5 = this.execSql(sql5);

        List<Map<String, Object>> list6 = this.execSql(sql6);
        List<Map<String, Object>> list7 = this.execSql(sql7);

        String dept = "SELECT ID,DEPTNAME,0 as wks ,0 as ksz ,0 as ywc FROM RZTSYSDEPARTMENT WHERE ORGTYPE = 0";
