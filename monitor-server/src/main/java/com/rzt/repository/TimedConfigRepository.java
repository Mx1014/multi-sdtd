...
public interface TimedConfigRepository  extends JpaRepository<TimedConfig, String> {
    ...
    @Modifying
    @Query(value = " UPDATE  WORKING_TIMED SET START_TIME = ?2 , END_TIME = ?3  WHERE DEPT_ID = ?1", nativeQuery = true)
    int updateWorkings(String deptId, String startTime, String endTime);
}
