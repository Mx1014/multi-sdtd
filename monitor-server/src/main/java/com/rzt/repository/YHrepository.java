...
public interface YHrepository  extends JpaRepository<KhYhHistory, String> {
    ...
    @Modifying
    @Query(value = "UPDATE XS_SB_YH SET YHMS = ?2 , YHTDQX = ?3 ,YHTDXZJD = ?4 , YHTDC = ?5 ,GKCS = ?6 ,JSP = ?7 WHERE ID = ?1",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "INSERT INTO ACT_TASK_INFO VALUES (?1,?3,?2,?4,?8,?5,?6,?7,?9,?10)", nativeQuery = true)
    int inserActTaskInfo(String id, String proKey, String proId, String userId, String flag, String info, String assignee, Date date,String iskh,String isjc);
}
